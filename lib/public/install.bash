...
install_1password_cli_linux_ubuntu() {
	local package="1Password-CLI"
	local deb_package="1password-cli"
...
add_tailscale_ppa_linux_ubuntu() {
	local ppa="tailscale"
	local signing_key_url="https://pkgs.tailscale.com/stable/ubuntu/$(lsb_release -cs).gpg"
	local ppa_url="https://pkgs.tailscale.com/stable/ubuntu"
	local pgp_public_key="/etc/apt/keyrings/${ppa}.asc"
	...
	wget -q -O - ${signing_key_url} | sudo tee ${pgp_public_key} >/dev/null
	echo "deb [arch=amd64 signed-by=${pgp_public_key}] ${ppa_url} $(lsb_release -cs) main" | sudo tee /etc/apt/sources.list.d/${ppa}.list >/dev/null
...
install_tailscale_linux_ubuntu() {
	local package="tailscale"
	local deb_package="tailscale"
	...
	info "Installing ${package} (${deb_package})..."
	# Assume called: add_tailscale_ppa_linux_ubuntu && sudo apt update
	sudo apt install ${deb_package} -y
